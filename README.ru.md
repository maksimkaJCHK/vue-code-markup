...
*   **MuStyleKey** - если нужно вывести код препроцессора (less или sass), то этот компонент нужен для создания вложенных правил. К примеру, если у нас в классе ".book", лежит компонент "&-item", то его нужно добавить в этот компонент.
*   **MuStyleParam** - в этот компонент нужно будет положить свойства CSS элемента. такими свойствами являются margin, padding, font-size и тд.
*   **MuStyleUnitMeas** - этот покпонент нужен для вывода единиц измерений (em. rem, px и тд).
